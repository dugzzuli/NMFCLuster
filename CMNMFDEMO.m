...
label_cluter=load( 'G:\\数据\\多变量事件序列数据集\\Network\\1000\\data4\\data4label.txt');
...
for min_count=6:6
    ...
    for count=1:1000
        path=sprintf('G:\\数据\\多变量事件序列数据集\\Network\\1000\\data4\\calc_net\\%d\\',count);
        ...
        [W_re,H_re,err,err_data]=CMNMF(VData,r,tol,ite,change,lambda,0,'svd');
    ...
    save result_rnmfCMNMF.txt -ascii minSave
end
